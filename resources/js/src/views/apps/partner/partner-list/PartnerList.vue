    ...
    >
        ...
        <b-row>
          ...
          >
            <div class="d-flex align-items-center justify-content-end">
              ...
              <b-button
                variant="primary"
                @click="isAddNewPartnerSidebarActive = true"
              >
                <span class="text-nowrap">Add Partner</span>
              </b-button>
            </div>
          </b-col>
        </b-row>
      ...
      >
          ...
          <template #cell(id)="data">
              <b-link
                  :to="{ name: 'apps-admin-partner-view', params: { id: data.item.id }}"
              ...
              <b-media vertical-align="center">
                  <template #aside>
                      <b-avatar
                          ...
                          :variant="`light-${resolvePartnerAvatarVariant(data.item.status)}`"
                      />
                  </template>   {{ data.item.company.name }}
        ...
        <template #cell(role)="data">
          <div class="text-nowrap">
            <feather-icon
              :icon="resolvePartnerRoleIcon(data.item.role)"
              size="18"
              class="mr-50"
              :class="`text-${resolvePartnerRoleVariant(data.item.role)}`"
            />
          ...
          <template #cell(status)="data">
              <b-badge
                  pill
                  :variant="`light-${resolvePartnerStatusVariant(data.item.status)}`"
        ...
        <template #cell(actions)="data">
          ...
          >
            ...
            <b-dropdown-item :to="{ name: 'apps-admin-partner-view', params: { id: data.item.id } }">
              <feather-icon icon="FileTextIcon" />
              <span class="align-middle ml-50">Details</span>
            </b-dropdown-item>

            <b-dropdown-item :to="{ name: 'apps-admin-partner-edit', params: { id: data.item.id } }">
            ...
            <b-dropdown-item>
              ...
              <span class="align-middle ml-50" @click="deletePartner(data.item.id)">Delete</span>
            </b-dropdown-item>
          </b-dropdown>
        </template>
...
import PartnersListFilters from './PartnerListFilters.vue'
import usePartnersList from './usePartnerList'
  ...
  setup() {
      ...
      function deletePartner(id) {
          this.$swal({
              ...
              if (result.value) {
                  store.dispatch('app-partners/deletePartner', {id: id})
                  this.$swal({
                      icon: 'success',
                      title: 'Deleted!',
