...
export default {
  ...
  setup() {
    const INVOICES = 'app-invoice'

    // Register module
    if (!store.hasModule(INVOICES))
      store.registerModule(INVOICES, invoiceStoreModule)

    // UnRegister on leave
    onUnmounted(() => {
      if (store.hasModule(INVOICES)) store.unregisterModule(INVOICES)
    })
