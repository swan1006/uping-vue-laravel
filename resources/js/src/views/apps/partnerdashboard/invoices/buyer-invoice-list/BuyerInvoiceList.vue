...
import useBuyerInvoicesList from './useBuyerInvoiceList'
...
export default {
  ...
  setup() {
    ...
    const buyerInvoiceData = ref(null)
    ...
    const statusOptions = ['Downloaded', 'Draft', 'Paid', 'Partial Payment', 'Past Due']
      ...
      store.dispatch('app-invoice/fetchBuyerInvoices', { id: router.currentRoute.params.id })
          .then(response => {
              // console.log(response.data.invoices)
              buyerInvoiceData.value = response.data.invoices })
          .catch(error => {
              console.log(error)
