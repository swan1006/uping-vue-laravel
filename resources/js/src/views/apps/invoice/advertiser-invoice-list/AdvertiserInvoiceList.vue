<template>
    ...
    >
        ...
        <!--      :items="fetchInvoices('', '', {id: data.item.id})"-->
        <b-table
            ref="refInvoiceListTable"
            responsive
            :items="advertiserInvoiceData"
            :fields="tableColumns"
            primary-key="id"
            :sort-by.sync="sortBy"
            show-empty
...
<script>
    ...
    import useAdvertiserInvoicesList from './useAdvertiserInvoiceList'
    ...
    export default {
        ...
        setup() {
            ...
            const advertiserInvoiceData = ref(null)
            ...
            store.dispatch('app-invoice/fetchAdvertiserInvoices', { id: router.currentRoute.params.id })
                .then(response => {
                    // console.log(response.data.invoices)
                    advertiserInvoiceData.value = response.data.invoices })
                .catch(error => {
                    console.log(error)
                    if (error.response.status === 404) {
                        advertiserInvoiceData.value = undefined
                    }
                })
